...
-- ========== 信号层扩展表（由 signal_layer_ext_ddl.sql 合并，数据采集器写入这些表） ==========

CREATE TABLE IF NOT EXISTS `signal_concept_block` (
...
CREATE TABLE IF NOT EXISTS `signal_fund_flow` (
    `id`               BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
    `stock_code`       VARCHAR(10)    NOT NULL COMMENT '股票代码',
...
CREATE TABLE IF NOT EXISTS `signal_dragon_tiger_detail` (
    `id`               BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
...
CREATE TABLE IF NOT EXISTS `signal_lockup_detail` (
    `id`               BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
    `stock_code`       VARCHAR(10)    NOT NULL COMMENT '股票代码',
